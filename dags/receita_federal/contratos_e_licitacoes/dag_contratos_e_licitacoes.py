...
from receita_federal.contratos_e_licitacoes import get_contratos
from receita_federal.contratos_e_licitacoes import get_licitacoes
...
# Definindo a DAG
with DAG(
    dag_id='contratos_e_licitacoes',
    start_date=days_ago(1),
    schedule_interval=None,
    default_args=default_args,
    tags=['contratos', 'licitacoes','trusted','receita federal']
) as dag:

    # Definindo o task que carrega a tabela na trusted (tratamentos iniciais)
    task1 = PythonOperator(
        task_id = 'csv_to_delta_contratos',
        python_callable = get_contratos.get_bases_contratos,
        provide_context=True
    )

    # Definindo o task que carrega a tabela na trusted (tratamentos iniciais)
    task2 = PythonOperator(
        task_id = 'csv_to_delta_licitacoes',
        python_callable = get_licitacoes.get_bases_licitacoes,
        provide_context=True
    )
