...
# SCRIPTS
from scripts.receita_federal.receita_fato_dimensao.motivos_raw_to_trusted import motivos_to_trusted

# DEFINE VARIABLES
MINIO_CONN_RAW = "minio_raw"
MINIO_RAW_BUCKET = "receita-federal"
RECEITA_FEDERAL_MOTIVOS_FOLDER = "motivos/"


now = datetime.now(tz=timezone(timedelta(hours=-3)))
yesterday = now - timedelta(days=1)
day_to_process_motivos = f"{RECEITA_FEDERAL_MOTIVOS_FOLDER}year=2024/month=6/"#'#year={yesterday.year}/month={yesterday.month}/day={yesterday.day}/"year={yesterday.year}/month={yesterday.month}/day={yesterday.day}/"
...
def receita_federal_motivos():
    # init & finish task
    init_data_load = EmptyOperator(task_id="init")
    finish_data_load = EmptyOperator(task_id="finish")

    list_today_files_motivos = S3ListOperator(
        task_id="list_today_files_motivos",
        aws_conn_id=MINIO_CONN_RAW,
        bucket=MINIO_RAW_BUCKET,
        prefix=day_to_process_motivos,
        apply_wildcard=True,
    )
    ...
    check_files = ShortCircuitOperator(
        ...
        op_kwargs={'files_to_process': list_today_files_motivos.output}
    )
    ...
    def motivos(current_files_motivos):
        ...
        print(f"current_files_motivos as { type(current_files_motivos) } and size of { len(current_files_motivos) }")

        motivos_to_trusted(current_files_motivos, access_params)

    unique_clients = motivos(list_today_files_motivos.output)

    # run order
    init_data_load >> list_today_files_motivos >> check_files >> unique_clients >> finish_data_load

receita_federal_motivos()