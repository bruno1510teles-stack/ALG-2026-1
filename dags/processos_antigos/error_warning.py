...
def erro(context):
    ...
    errorBody = {
        ...
        "error.stack_trace": f"Erro na execução da DAG {context['dag'].dag_id}",
        "container": {
        ...
        "error.message": f"dag_run_id: {context['dag_run'].run_id}",
        "log.level": "ERROR",
        "message": f"Erro na execução da DAG {context['dag'].dag_id}",
        "input": {
            "type": "container"
        },
        "cloud": {
