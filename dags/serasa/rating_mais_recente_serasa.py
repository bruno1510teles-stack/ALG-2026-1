...
def rating_mais_recente(access_params=None,  **kwargs):
    ...
    # Query Propostas
    query_propostas = f"""
    -- CTE 1: Seleciona todas as propostas
    WITH propostas AS (
        SELECT 
            p.data_criado,                
            p.data_resolvido,             
            ...
            ROW_NUMBER() OVER (
                ...
                ORDER BY (p.data_resolvido) DESC                                                -- Mantém a última proposta do CNPJ raiz
            ) AS rn
    ...
    -- CTE 2: Junta faturamento estimado ao cliente (última proposta)
    propostas_com_faturamento AS (
        SELECT 
            p.*,                           -- Todas as colunas da CTE anterior
            f.faturamento_estimado           -- Faturamento estimado do cliente
        FROM propostas p
        LEFT JOIN deltalakerefined.motor.faturamento_estimado f
        ON p.cnpj_raiz = f.cnpj_raiz
        WHERE p.rn = 1                     -- Apenas a última proposta de cada cliente
    ),
    ...
    print(f"Quantidade de linhas df_propostas_com_score: {propostas_enriquecidas.shape[0]}")
   ...
   # Regras parte da política desafiante
    ...
    df_rating_atual = base_analisar.apply(aplicar_regra_certa, axis=1)

    # Tratamento base
    df_rating_atual['ramificacao_final'] = df_rating_atual['ramificacao'] + ' | ' + df_rating_atual['ramificacao_2']
    ...
    if df_rating_atual is not None and not df_rating_atual.empty:
        # Aplicar a função nas colunas desejadas
        df_rating_atual['limite_pedido'] = df_rating_atual['limite_pedido'].apply(ajustar_decimal)
        df_rating_atual['limite_aprovado'] = df_rating_atual['limite_aprovado'].apply(ajustar_decimal)
        df_rating_atual['total_restritivos_pj'] = df_rating_atual['total_restritivos_pj'].apply(ajustar_decimal)
        df_rating_atual['total_restritivos_pf'] = df_rating_atual['total_restritivos_pf'].apply(ajustar_decimal)
        df_rating_atual['valor_total_restritivos'] = df_rating_atual['valor_total_restritivos'].apply(ajustar_decimal)
    ...
    df_rating_atual[cols_float] = df_rating_atual[cols_float].apply(pd.to_numeric, errors='coerce').round(2)

    # Colunas de texto
    colunas_string = ['razao_social', 'parecer', 'id']

    for coluna in colunas_string:
        df_rating_atual[coluna] = df_rating_atual[coluna].fillna('').astype('string')


    # Colunas de data
    # Converter data_consulta para datetime64[ns, UTC], depois extrai somente a data, esse campo já é date, apenas para garantir
    df_rating_atual['data_consulta'] = pd.to_datetime(df_rating_atual['data_consulta'], errors='coerce', utc=True).dt.date

    # Extrair apenas a data para todas as colunas, já são datetime64[ns, UTC]
    df_rating_atual['data_criado'] = df_rating_atual['data_criado'].dt.date
    df_rating_atual['data_resolvido'] = df_rating_atual['data_resolvido'].dt.date
    ...
    for col in colunas_int:
        df_rating_atual[col] = (
            pd.to_numeric(df_rating_atual[col], errors='coerce')  # converte para numérico com NaNs
            .round(0)                                               # arredonda para zero casas decimais
            .astype('Int64')                                        # converte para inteiro com suporte a nulos
        )

    # Remove colunas
    df_rating_atual = df_rating_atual.drop(
    ...
    df_rating_atual = df_rating_atual[ordem_colunas].reset_index(drop=True)

    # Colunas de data
    # Timestamp e partições
    now = datetime.now(tz=timezone(timedelta(hours=-3)))
    df_rating_atual['atualizado_em'] = now.strftime('%Y-%m-%d %X')
    df_rating_atual['year'], df_rating_atual['month'], df_rating_atual['day'] = now.year, now.month, now.day
