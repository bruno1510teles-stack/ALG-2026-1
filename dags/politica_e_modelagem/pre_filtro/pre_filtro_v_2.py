...
def analise_pre_filtro(access_params=None,  **kwargs):
    ...
    for index, row in base_analisar_raiz.iterrows():
        ...
        query = (f"""
            ...
            with limite as (
				select 
					cnpj_raiz, limite_alpe, NULLIF(limite_disponivel, 0) / NULLIF(limite_atribuido, 0) AS pcto_limite_utilizado, situacao_sacado, limite_atribuido
				from (
	                 select 
    ...
    response_map = {
        ...
        'mantido' : ['PF LIMITE SOLICITADO <= ATUAL', 'PF - UTILIZAÇÃO DE LIMITE MÍNIMA NÃO ATINGIDA', 'PF BLOQUEIO ALPE'],
