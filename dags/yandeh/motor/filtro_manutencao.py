...
def def_filtro_manutencao_yandeh(access_params=None,  **kwargs):
    ...
    query = f"""
        with
        base_clientes_ativos as (
        select 
            distinct regexp_replace(cnpj, '[./-]', '') AS cnpj_sacado, substring(regexp_replace(cnpj, '[./-]', ''), 1,8) AS cnpj_raiz 
        from 
            s3alpeyandeh.yandeh.rm_boleto 
        where  
            data_emissao >= current_date - INTERVAL '3' month
        union 
        select
            distinct regexp_replace(cnpj_sacado, '[./-]', '') as cnpj_sacado, substring(regexp_replace(cnpj_sacado, '[./-]', ''), 1,8) AS cnpj_raiz 
        from 
        ...
        select
            bc.cnpj_sacado, he.grupo_economico 
        from 
            base_clientes_ativos bc
            left join s3alpeyandeh.yandeh.hubspot_empresas he on bc.cnpj_sacado = he.cnpj 
        ),
        base_limites as (
        select
        ...
        base_consolidada as (
        select 
            bc.cnpj_sacado, bc.cnpj_raiz, nge.grupo_economico, coalesce(bl.limite_alpe, 0) as limite_alpe, coalesce(bl.limite_yandeh,0) as limite_yandeh, coalesce(bd.badlist, 0) as badlist,  coalesce(bia.inadimplente_alpe, 0) as inadimplente_alpe, 
        ...
        from 
            ...
            left join base_limites bl on bc.cnpj_sacado = bl.cnpj_sacado 
