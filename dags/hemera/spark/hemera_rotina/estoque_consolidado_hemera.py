...
def estoque_consolidado(spark):
    ...
    # Tratamento Valor Presente

    df = df.withColumn(
        "valor_presente_vendermais",
        F.when(F.col("grupo") == "VenderMais" , F.col("valor_presente"))
        .otherwise(0)
    )

    df = df.withColumn(
        "valor_presente_tradicional",
        F.when(F.col("grupo") == "Tradicional" , F.col("valor_presente"))
        .otherwise(0)
    )

    df = df.withColumn(
        "valor_presente_conglomerado",
        F.when(F.col("grupo") == "Conglomerado" , F.col("valor_presente"))
        .otherwise(0)
    )
