...
from laudos_alpe_historico import raw_laudos
...
# Definindo a DAG
with DAG(
    dag_id='laudos_historico',
    start_date=days_ago(1),
    schedule_interval='0 9 * * 0-5',
    default_args=default_args,
    tags=['etl', 'laudos', 'trusted'],
    max_active_runs=1
) as dag:

    # Definindo o task que atualiza a tabela laudos
    laudos_alpe_historico = PythonOperator(
        task_id='laudos_consolidado_trusted',
        python_callable= raw_laudos,
        op_kwargs={'access_params': access_params},
        provide_context=True  # Habilita o envio do contexto (incluindo conf)
    )

    # Definindo a ordem de execução das tasks
    laudos_alpe_historico