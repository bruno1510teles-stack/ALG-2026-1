...
# SCRIPTS
from scripts.respostas_credito.motor.respostas_motor_to_trusted import transform_motor_to_trusted
...
day_to_process_motor = f"{RESPOSTA_MOTOR_RAW_FOLDER}year={yesterday.year}/"
...
def respostas_credito_motor():
    ...
    def transform_raw_to_trusted(current_files_motor):
        ...
        transform_motor_to_trusted(current_files_motor, access_params)

    unique_clients = transform_raw_to_trusted( list_today_files_motor.output)

    # run order
    init_data_load >> list_today_files_motor >> check_files_motor >> unique_clients >> finish_data_load

respostas_credito_motor()