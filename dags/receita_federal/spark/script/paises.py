...
def paises_to_trusted(spark):
    ...
    trusted_paises = trusted_paises.withColumn("data_ref", lit(data_ref))

    now = datetime.now(tz=timezone(timedelta(hours=-3)))
    atualizado_em = now.strftime('%Y-%m-%d %X')  
    trusted_paises = trusted_paises.withColumn("atualizado_em", lit(atualizado_em))

    print(f"DataFrame carregado. Esquema: {trusted_paises.printSchema()}")
    print(f"Primeiras linhas do DataFrame: {trusted_paises.show(5)}") 
    row_count = trusted_paises.count()
