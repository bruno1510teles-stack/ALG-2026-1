...
def hemera_raw_to_trusted(access_params=None, **kwargs):
    ...
    FOLDER_DESTINATION_RAW = 'recompra/year=2024/'
    ...
    # Filtrando colunas
    colunas_desejadas = ['CedenteCnpjCpf', 'CedenteNome', 'SacadoCnpjCpf', 'SacadoNome',
        'IdTituloVx', 'TipoAtivo', 'DataEmissao', 'DataAquisicao',
        'DataVencimento', 'NumeroBoletoBanco', 'ValorPagamento', 'data_arquivo', 'DataLancamento', 'data_ref']

    # Filtrando no DataFrame
    df_consolidado = df_consolidado[colunas_desejadas]

    df_consolidado = df_consolidado.loc[df_consolidado['ValorPagamento'] > 0]
    ...
    colunas_para_converter_datetime = ['DataVencimento', 'DataEmissao', 'DataAquisicao', 'data_fechamento', 'DataLancamento']
    df_consolidado = converter_para_datetime(df_consolidado, colunas_para_converter_datetime)

    # Agrupando dados

    df_agrupado = df_consolidado.groupby(['data_ref', 'IdTituloVx', 'data_fechamento']).agg(
        valor_pagamento=('ValorPagamento', 'sum'),  
        data_lancamento=('DataLancamento', 'max')
    ).reset_index()
    ...
    FOLDER_DESTINATION_TRUSTED = "recompra"
