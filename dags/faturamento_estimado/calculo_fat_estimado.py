...
def calcular_faturamento_estimado (spark, **kwargs):
    ...
    query_fat_serasa = f"""
                        ...
                        from postgres.exrp_{Variable.get('STAGE')}_default.report r
                                ...
                                inner join (select document_number, max(id) as max_id from postgres.exrp_{Variable.get('STAGE')}_default.identification_report group by document_number) ir2 on ir2.max_id = r.identification_report_id
                                ...
                                inner join postgres.exrp_{Variable.get('STAGE')}_default.period total on total.id = ts.total_id
                        """
