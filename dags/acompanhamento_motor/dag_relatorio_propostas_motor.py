...
from acompanhamento_motor.relatorio_propostas_motor import base_prop_decididas
from acompanhamento_motor.msg_teams import enviar_mensagem_teams
...
with DAG(
    dag_id='relatorio_propostas_motor',
    start_date=days_ago(1),
    schedule_interval = '0 13 * * *',  # Roda todos os dias às 10:00
    default_args=default_args,
    tags=['relatorio', 'propostas', 'motor'],
    max_active_runs=1
) as dag:

    enviando_arquivo_ao_minio = PythonOperator(
        task_id='envio_arquivo',
        python_callable=base_prop_decididas,
        op_kwargs={'access_params': access_params},
        provide_context=True  
    )

    mensagem_teams = PythonOperator(
        task_id='aviso_teams',
        python_callable=enviar_mensagem_teams,
        op_kwargs={'access_params': access_params},
        provide_context=True  
    )

    enviando_arquivo_ao_minio >> mensagem_teams