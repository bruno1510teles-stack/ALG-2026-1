...
def motivos_to_trusted(spark):
    ...
    # Baixando arquivos Raw
    trusted_motivos = spark.read \
        .option("delimiter", ";") \
        .option("header", False)\
    ...
    # Renomeando colunas    
    trusted_motivos = trusted_motivos \
    ...
    trusted_motivos = trusted_motivos.withColumn("data_ref", lit(data_ref))

    now = datetime.now(tz=timezone(timedelta(hours=-3)))
    atualizado_em = now.strftime('%Y-%m-%d %X')  
    trusted_motivos = trusted_motivos.withColumn("atualizado_em", lit(atualizado_em))

    print(f"DataFrame carregado. Esquema: {trusted_motivos.printSchema()}")
    print(f"Primeiras linhas do DataFrame: {trusted_motivos.show(5)}") 
    row_count = trusted_motivos.count()
    ...
    trusted_motivos.write \
        .partitionBy("data_ref") \
        .format("delta") \
        .option("mergeSchema", "true") \
        .option("encoding", 'latin1') \
        .mode("overwrite") \
        .save("s3a://teste-felipe/receita-federal/motivos")
