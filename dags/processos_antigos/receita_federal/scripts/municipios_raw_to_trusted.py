...
# Criando conexão
def municipios_to_trusted(files_list_municipios, access_params):

    # Variaveis Conexão
    BUCKET_SOURCE_RAW = "receita-federal"
    BUCKET_SOURCE_TRUSTED = "receita-federal"
    TRUSTED_FOLDER = "municipios/"
    ...
    print(f'IMPORTANDO municipios {psutil.virtual_memory()._asdict()}')

    for file_name in files_list_municipios:
