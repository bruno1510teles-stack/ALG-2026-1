...
# SCRIPTS
from scripts.receita_federal.relacionamento_pessoas_e_organizacoes import transform_receita_to_relacionamento
...
day_to_process_socios = f"{RECEITA_FEDERAL_SOCIOS_FOLDER}year=2024/month=4/" #f"{RECEITA_FEDERAL_SOCIOS_FOLDER}year={yesterday.year}/month={str(yesterday.month)}/"
...
def relacionamento_receita_federal():
    # init & finish task
    ...
    list_today_files_socio = S3ListOperator(
        task_id="list_today_files_socio",
        aws_conn_id=MINIO_CONN_RAW,
        bucket=MINIO_RAW_BUCKET,
        prefix=day_to_process_socios,
        apply_wildcard=True,
    )
    ...
    def relacionamento_receita_federal(current_files_estabelecimento, current_files_socio):
        ...
        print(f"current_files_socio as { type(current_files_socio) } and size of { len(current_files_socio) }")
        
        transform_receita_to_relacionamento(current_files_estabelecimento, current_files_socio, access_params)

    unique_clients = relacionamento_receita_federal(list_today_files_estabelecimento.output, list_today_files_socio.output)

    # run order
    init_data_load >> list_today_files_estabelecimento >> check_files >> list_today_files_socio >> unique_clients >> finish_data_load

relacionamento_receita_federal()