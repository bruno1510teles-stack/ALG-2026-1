...
def retorno_diario_trusted (access_params=None, **kwargs):
    ...
    retorno_atual = execute_query(conn, query_retorno)
    ...
    retorno_atual['data_arquivo'] = pd.to_datetime(retorno_atual['data_arquivo']).dt.tz_localize(None).dt.floor('S')

    print('Linhas antes da remoção:')
    print(len(retorno_atual))

    # Remover do DataFrame as linhas com essas datas
    retorno_atual = retorno_atual[~retorno_atual['data_arquivo'].isin(datas_para_remover)]
    ...
    retorno_final = pd.concat([retorno_atual, final_df], ignore_index=True)

    print('Linhas após a inserção de novas linhas:')
    print(len(retorno_final))

    # Garantindo que não vou ter tidos de dados invalidos para o DeltaLake
    for col in retorno_final.columns:
        if pd.api.types.is_numeric_dtype(retorno_final[col]):
            retorno_final[col] = retorno_final[col].fillna(0)
        elif pd.api.types.is_datetime64_any_dtype(retorno_final[col]):
            retorno_final[col] = retorno_final[col].fillna(pd.NaT)
        else:
            retorno_final[col] = retorno_final[col].fillna('')
    ...
    retorno_final = converter_colunas_data(retorno_final).copy()
