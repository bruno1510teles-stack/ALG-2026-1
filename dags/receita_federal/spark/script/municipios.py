...
def municipios_to_trusted(spark):
    ...
    trusted_municipios = trusted_municipios \
    ...
    trusted_municipios = trusted_municipios.withColumn("data_ref", lit(data_ref))

    now = datetime.now(tz=timezone(timedelta(hours=-3)))
    atualizado_em = now.strftime('%Y-%m-%d %X')  
    trusted_municipios = trusted_municipios.withColumn("atualizado_em", lit(atualizado_em))
    
    print(f"DataFrame carregado. Esquema: {trusted_municipios.printSchema()}")
    print(f"Primeiras linhas do DataFrame: {trusted_municipios.show(5)}") 
    row_count = trusted_municipios.count()
