...
# Criando conexão
def transform_receita_to_natureza_juridica(files_list_estabelecimento, files_list_empresa, files_list_natureza_juridica, access_params):

    # Variaveis Conexão
    BUCKET_SOURCE_RAW = "receita-federal"
    BUCKET_SOURCE_TRUSTED = "pessoas-e-organizacoes"
    TRUSTED_FOLDER = "natureza_juridica/"
    ...
    #importando naturezas juridicas
    print("importando Natjur")
    file_name = files_list_natureza_juridica[0]
    file = client.get_object(bucket_name=BUCKET_SOURCE_RAW, object_name=file_name)
    df_natureza_juridica = pd.read_csv(BytesIO(file.data), dtype='str', sep=';', header=None, encoding='latin1')

    #transformando df em dict para posterior map
    dict_natureza_juridica = dict(zip(df_natureza_juridica[0], df_natureza_juridica[1]))
    ...
    for file_name in files_list_empresa:
        ...
        # Renomeando colunas com o nome padrão da Receita
        colunas_empresa = {0: 'CNPJ BÁSICO',
        2: 'codigo'}    
        
        df_empresa = df_empresa.rename(columns=colunas_empresa)
        
        df_empresa['descricao'] = df_empresa['codigo'].map(dict_natureza_juridica)
        ...
        dtype_estabelecimentos = {0:'string', 1:'string', 2:'string'}
        ...
        for file_name in files_list_estabelecimento:
            ...
            for df_estabelecimentos in pd.read_csv(BytesIO(file.data), sep=';', 
                ...
                df_final = df_estabelecimentos.merge(df_empresa, how='inner',  on='CNPJ BÁSICO')
                
                df_final['identificador'] = df_final['CNPJ BÁSICO'].astype(str).str.cat(
                    [df_final['CNPJ ORDEM'].astype(str), df_final['CNPJ DV'].astype(str)], sep='')

                df_final = df_final.drop(columns=['CNPJ BÁSICO', 'CNPJ ORDEM', 'CNPJ DV'])
                
                print(f"ordenando coluna final {psutil.virtual_memory()._asdict()}")
                df_final = df_final[['identificador', 'codigo', 'descricao']]
                ...
                df_final['fonte'] = 'RECEITA FEDERAL'
