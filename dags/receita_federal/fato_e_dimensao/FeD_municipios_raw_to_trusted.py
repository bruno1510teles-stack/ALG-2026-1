...
# SCRIPTS
from scripts.receita_federal.receita_fato_dimensao.municipios_raw_to_trusted import municipios_to_trusted

# DEFINE VARIABLES
MINIO_CONN_RAW = "minio_raw"
MINIO_RAW_BUCKET = "receita-federal"
RECEITA_FEDERAL_MUNICIPIOS_FOLDER = "municipios/"


now = datetime.now(tz=timezone(timedelta(hours=-3)))
yesterday = now - timedelta(days=1)
day_to_process_municipios = f"{RECEITA_FEDERAL_MUNICIPIOS_FOLDER}year=2024/month=6/"#'#year={yesterday.year}/month={yesterday.month}/day={yesterday.day}/"year={yesterday.year}/month={yesterday.month}/day={yesterday.day}/"
...
def receita_federal_municipios():
    # init & finish task
    init_data_load = EmptyOperator(task_id="init")
    finish_data_load = EmptyOperator(task_id="finish")

    list_today_files_municipios = S3ListOperator(
        task_id="list_today_files_municipios",
        aws_conn_id=MINIO_CONN_RAW,
        bucket=MINIO_RAW_BUCKET,
        prefix=day_to_process_municipios,
        apply_wildcard=True,
    )
    ...
    check_files = ShortCircuitOperator(
        ...
        op_kwargs={'files_to_process': list_today_files_municipios.output}
    )
    ...
    def municipios(current_files_municipios):
        ...
        print(f"current_files_municipios as { type(current_files_municipios) } and size of { len(current_files_municipios) }")

        municipios_to_trusted(current_files_municipios, access_params)

    unique_clients = municipios(list_today_files_municipios.output)

    # run order
    init_data_load >> list_today_files_municipios >> check_files >> unique_clients >> finish_data_load

receita_federal_municipios()