...
# Criando conexão
def cnaes_to_trusted(files_list_cnaes, access_params):

    # Variaveis Conexão
    BUCKET_SOURCE_RAW = "receita-federal"
    BUCKET_SOURCE_TRUSTED = "receita-federal"
    TRUSTED_FOLDER = "cnaes/"
    ...
    # Importando dados de CNAES
    print(f'IMPORTANDO CNAES {psutil.virtual_memory()._asdict()}')

    for file_name in files_list_cnaes:
        ...
        df['mes_ref'] = file_name[-12:-10]
        df['ano_ref'] = str(datetime.now().year)[0:3] + file_name[-13:-12]
