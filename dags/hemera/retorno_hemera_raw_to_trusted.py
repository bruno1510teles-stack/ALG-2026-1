...
def hemera_raw_to_trusted(access_params=None, **kwargs):
    ...
    FOLDER_DESTINATION_RAW = 'retorno/year=2025/month=01'
    ...
    colunas_para_converter_datetime = ['DataVencimento', 'data_fechamento', 'data_arquivo']
    df_consolidado = converter_para_datetime(df_consolidado, colunas_para_converter_datetime)


    # Agrupando dados

    df_agrupado = df_consolidado.groupby(['data_ref','ID_Registro_VX']).agg(
        valor_pagamento=('ValorPagamento', 'sum'),  
        data_lancamento=('data_arquivo', 'max'),      
        data_fechamento=('data_fechamento', 'max')
    ).reset_index()
    ...
    FOLDER_DESTINATION_TRUSTED = "retorno"
