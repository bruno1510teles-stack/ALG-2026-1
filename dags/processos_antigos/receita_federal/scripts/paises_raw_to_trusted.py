...
# Criando conexão
def paises_to_trusted(files_list_paises, access_params):

    # Variaveis Conexão
    BUCKET_SOURCE_RAW = "receita-federal"
    BUCKET_SOURCE_TRUSTED = "receita-federal"
    TRUSTED_FOLDER = "paises/"
    ...
    # Importando dados de paises
    print(f'IMPORTANDO paises {psutil.virtual_memory()._asdict()}')

    for file_name in files_list_paises:
