...
# SCRIPTS
from scripts.receita_federal.receita_fato_dimensao.paises_raw_to_trusted import paises_to_trusted

# DEFINE VARIABLES
MINIO_CONN_RAW = "minio_raw"
MINIO_RAW_BUCKET = "receita-federal"
RECEITA_FEDERAL_PAISES_FOLDER = "paises/"


now = datetime.now(tz=timezone(timedelta(hours=-3)))
yesterday = now - timedelta(days=1)
day_to_process_paises = f"{RECEITA_FEDERAL_PAISES_FOLDER}year=2024/month=7/day=26/"#year={yesterday.year}/month={yesterday.month}/day={yesterday.day}/"
...
def receita_federal_paises():
    # init & finish task
    init_data_load = EmptyOperator(task_id="init")
    finish_data_load = EmptyOperator(task_id="finish")

    list_today_files_paises = S3ListOperator(
        task_id="list_today_files_paises",
        aws_conn_id=MINIO_CONN_RAW,
        bucket=MINIO_RAW_BUCKET,
        prefix=day_to_process_paises,
        apply_wildcard=True,
    )
    ...
    check_files = ShortCircuitOperator(
        ...
        op_kwargs={'files_to_process': list_today_files_paises.output}
    )
    ...
    def paises(current_files_paises):
        ...
        print(f"current_files_paises as { type(current_files_paises) } and size of { len(current_files_paises) }")

        paises_to_trusted(current_files_paises, access_params)

    unique_clients = paises(list_today_files_paises.output)

    # run order
    init_data_load >> list_today_files_paises >> check_files >> unique_clients >> finish_data_load

receita_federal_paises()