...
with DAG(
    ...
    # Sensor que aguarda a conclusão da DAG hemera_diario
    esperando_hemera_diario = ExternalTaskSensor(
        task_id='esperando_hemera_diario',
        external_dag_id='hemera_diario',  # Nome da DAG que deve ser concluída
    ...
    aquisicao_consolidado = SparkKubernetesOperator(
        task_id='aquisicao_consolidado',
        application_file='aquisicao-consolidado-hemera-spark-app.yaml',
        namespace='spark',
        kubernetes_conn_id='kubernetes_default',
        do_xcom_push=True,
    )

    recompra_consolidado = SparkKubernetesOperator(
        task_id='recompra_consolidado',
        application_file='recompra-consolidado-hemera-spark-app.yaml',
        namespace='spark',
        kubernetes_conn_id='kubernetes_default',
        do_xcom_push=True,
    )

    retorno_consolidado = SparkKubernetesOperator(
        task_id='retorno_consolidado',
        application_file='retorno-consolidado-hemera-spark-app.yaml',
        namespace='spark',
        kubernetes_conn_id='kubernetes_default',
        do_xcom_push=True,
    )

    estoque_consolidado = SparkKubernetesOperator(
        task_id='estoque_consolidado',
        application_file='estoque-consolidado-v2-hemera-spark-app.yaml',
        namespace='spark',
        kubernetes_conn_id='kubernetes_default',
        do_xcom_push=True,
    )
    ...
    esperando_hemera_diario >> aquisicao_consolidado >> recompra_consolidado >> retorno_consolidado >> estoque_consolidado