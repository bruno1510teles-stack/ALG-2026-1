...
from cobranca.boletos import boletos_internos_to_trusted
...
# Definindo a DAG
with DAG(
    dag_id='boletos_internos',
    start_date=days_ago(1),
    schedule_interval='0 12,20 * * *',
    default_args=default_args,
    tags=['etl', 'boletos', 'internos','trusted'],
    max_active_runs=1
) as dag:
    
    
# Definindo o task que processa boletos tradicional raw to trusted
    boletos_internos_to_trusted_task = PythonOperator(
        task_id = 'boletos_internos_to_trusted',
        python_callable = boletos_internos_to_trusted.boletos_internos_to_trusted,
        op_kwargs = {'access_params': access_params},
        provide_context = True
    )

# Definindo a ordem de execução das tasks
    boletos_internos_to_trusted_task