...
def qualificacoes_to_trusted(spark):
    ...
    trusted_qualificacoes = spark.read \
        .option("delimiter", ";") \
        .option("header", False)\
    ...
    trusted_qualificacoes = trusted_qualificacoes \
    ...
    trusted_qualificacoes = trusted_qualificacoes.withColumn("data_ref", lit(data_ref))

    now = datetime.now(tz=timezone(timedelta(hours=-3)))
    atualizado_em = now.strftime('%Y-%m-%d %X')  
    trusted_qualificacoes = trusted_qualificacoes.withColumn("atualizado_em", lit(atualizado_em))

    print(f"DataFrame carregado. Esquema: {trusted_qualificacoes.printSchema()}")
    print(f"Primeiras linhas do DataFrame: {trusted_qualificacoes.show(5)}") 
    row_count = trusted_qualificacoes.count()
...
if __name__ == "__main__":
    spark = SparkSession.builder \
        .appName("qualificacoesToTrusted") \
    ...
    qualificacoes_to_trusted(spark)
