...
# SCRIPTS
from scripts.receita_federal.receita_fato_dimensao.socios_raw_to_trusted import socios_to_trusted
...
day_to_process_socios = f"{RECEITA_FEDERAL_SOCIOS_FOLDER}year=2024/month=7/day=26/"#year={yesterday.year}/month={yesterday.month}/day={yesterday.day}/"
...
def receita_federal_socios():
    ...
    def socios(current_files_empresa):
        ...
        socios_to_trusted(current_files_empresa, access_params)

    unique_clients = socios(list_today_files_empresa.output)

    # run order
    init_data_load >> list_today_files_empresa >> check_files >> unique_clients >> finish_data_load

receita_federal_socios()