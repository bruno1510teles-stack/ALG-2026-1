...
def consulta_relato():
    ...
    def busca_relato(**kwargs):
        ...
        valida_response_e_envia_arquivo(response=response_completo, base_url=base_url, headers=headers, cnpj=cnpj, bucket=bucket, tipo_relato='SERASA_RELATO')
        valida_response_e_envia_arquivo(response=response_quadro_social, base_url=base_url, headers=headers, cnpj=cnpj, bucket=bucket, tipo_relato='SERASA_RELATO_QUADRO_SOCIAL')
        valida_response_e_envia_arquivo(response=response_score, base_url=base_url, headers=headers, cnpj=cnpj, bucket=bucket, tipo_relato='SERASA_RELATO_SCORE')

    
    def envia_pdf(response, bucket, cnpj, tipo_relato):
    ...
    def is_relato_mais_antigo_que_60_dias(response):
        try:
    ...
    def valida_response_e_envia_arquivo(response, base_url, headers, cnpj, bucket, tipo_relato):
        if response and not is_relato_mais_antigo_que_60_dias(response):
            ...
            if response_pdf:
                envia_pdf(response=response_pdf, bucket=bucket, cnpj=cnpj, tipo_relato=tipo_relato)
            else:
