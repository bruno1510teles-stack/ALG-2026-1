...
sys.path.append('/opt/airflow/dags/repo/dags/bonus_inadimplencia')
from boletos_enriquecidos_inad import boletos_trusted_to_refined
...
# Definindo a DAG
with DAG(
    dag_id='bonus_inadimplencia',
    start_date=days_ago(1),
    schedule_interval= '30 11,20 * * *',
    default_args=default_args,
    tags=['etl', 'bonus', 'inadimplencia', 'refined'],
    max_active_runs=1
) as dag:

    # Definindo o task que atualiza a tabela de bônus de inadimplência
    bonus_inad = PythonOperator(
        task_id='boletos_enriquecidos_refined',
        python_callable= boletos_trusted_to_refined,
